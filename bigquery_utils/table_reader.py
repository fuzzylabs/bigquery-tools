...
class TableReader:
    ...
    def parallel_indexed_read(self, partition_count, output_dir, format='csv'):
    ...
    def parallel_partitioned_read(self, partition_count, output_dir, format='csv'):
        snapshot_time = int(time.time() * 1000)
        threads = []
        for index in range(partition_count):
            file_name = '%s.%d' % (os.path.join(output_dir, self.table_id), index)
...
class CSVResultHandler(FileResultHandler):

    def __init__(self, output_file_name, columns=None):
        FileResultHandler.__init__(self, output_file_name)
        self.csv_file = None
...
class TableReadThread(threading.Thread):
    ...
    def get_result_handler(self):
        if self.output_format.lower() == 'csv':
            ...
            return CSVResultHandler(self.output_file_name, self.get_columns())
...
def main(argv):
    ...
    if args.type == 'single-thread':
        thread = TableReadThread(table_reader, output_file_name, output_format=args.format)
        thread.start()
        thread.join()
    elif args.type == 'parallel-indexed':
        table_reader.parallel_indexed_read(output_dir=args.output_directory,
                                           partition_count=args.partition_count, format=args.format)
